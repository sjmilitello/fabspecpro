...
struct PieceEditorView: View {
    ...
    private func addCurve() {
        ...
        let segments = ShapePathBuilder.boundarySegments(for: piece).filter { $0.edge == defaultEdge }
        if let first = segments.first, let last = segments.last {
            curve.startCornerIndex = first.startIndex
            curve.endCornerIndex = last.endIndex
        }
        curve.piece = piece
...
private struct CurveRow: View {
    ...
    private func inferredEdgeLabel() -> String? {
        guard let edge = inferredEdgeFromSpan() else { return nil }
        return edge.label
    }
    ...
    private func inferredEdgeFromSpan() -> EdgePosition? {
        ...
        let bounds = ShapePathBuilder.displayPolygonPoints(for: piece, includeAngles: true)
        ...
        return edgeForSpanPoints(start: start, end: end, polygon: bounds, baseBounds: baseBounds)
    }
